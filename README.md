...
- **Video Worker** (FFmpeg) — processamento de vídeos: ffprobe (metadados/duração) + geração de thumbnail.
- **Object Storage** (MinIO/S3-compatible) — arquivos de vídeo e thumbnails; organizado como `channels/{channelId}/videos/{videoId}/original` e `.../thumbnail.jpg`.
- **Message Queue** (BullMQ + Redis) — fila `video-processing`; payload: `{ videoId, storageKey, channelId }`.
...
| MinIO (Object Storage) | http://localhost:9000 (API) / http://localhost:9001 (console) — credenciais: `minioadmin/minioadmin` |
| Redis | `localhost:6379` |
| Swagger (opcional) | http://localhost:3000/api/docs — habilite com `SWAGGER_ENABLED=true` |

### 1.1 Video Worker (opcional)

```bash
# Sobe o worker em modo watch (dentro do container video-worker)
docker compose exec video-worker npm run start:worker:dev
```

O worker consome a fila `video-processing`, extrai metadados com ffprobe e gera thumbnail com ffmpeg. Necessário para processar vídeos após o upload.
...
Upload de até 10GB via **presigned URL** (direto ao MinIO, sem passar pela API), processamento assíncrono com **BullMQ + Redis** e worker FFmpeg.

Ciclo de status: `draft` → `processing` → `ready` / `error`

Endpoints da API (`nestjs-project`):

| Método & Rota | Auth | Descrição |
|---------------|------|-----------|
| `POST /videos/upload-init` | JWT | Cria vídeo como rascunho, retorna presigned PUT URL (1h) |
| `POST /videos/:id/complete` | JWT (dono) | Marca upload concluído, enfileira processamento |
| `GET /videos/:uniqueId/stream` | Público | Streaming com suporte a Range/206 |
| `GET /videos/:uniqueId/download` | Público | Download com `Content-Disposition: attachment` |
...
│   │   ├── videos/                      # Upload, processamento, streaming e download
│   │   ├── storage/                     # StorageService (AWS S3 SDK → MinIO)
│   │   ├── queue/                       # QueueModule (BullMQ — fila video-processing)
...
│   ├── Dockerfile.dev
│   └── Dockerfile.worker.dev           # Worker FFmpeg
...
| Processamento de Vídeo | FFmpeg + fluent-ffmpeg, ffprobe |
| E-mail (dev) | Mailpit |
