...
npm run start:worker:dev                 # Video worker with hot-reload (inside video-worker container)
...
Required variables in `.env` (see `.env.example` for defaults):

| Variable | Description | Default (dev) |
|---|---|---|
| `DB_HOST` | PostgreSQL service name | `db` |
