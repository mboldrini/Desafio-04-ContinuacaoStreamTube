...
function makeChannelRepo(overrides: Partial<Repository<Channel>> = {}): any {
  return {
    findOne: jest.fn(),
    ...overrides,
  };
...
function makeService(
  manager: any,
  repoOverrides: Partial<Repository<Channel>> = {},
): ChannelsService {
  return new ChannelsService(
    makeDataSource(manager),
    makeChannelRepo(repoOverrides),
  );
}
...
describe('ChannelsService', () => {
  ...
  describe('findChannelByUserId', () => {
    it('returns channel for known userId', async () => {
      const channel = makeChannel('testchannel');
      const service = makeService(makeManager(), {
        findOne: jest.fn().mockResolvedValue(channel),
      });

      const result = await service.findChannelByUserId('user-id');

      expect(result).toBe(channel);
    });

    it('throws when no channel found for userId', async () => {
      const service = makeService(makeManager(), {
        findOne: jest.fn().mockResolvedValue(null),
      });

      await expect(service.findChannelByUserId('unknown-user')).rejects.toThrow(
        'Channel not found for user unknown-user',
      );
    });
  });
});
