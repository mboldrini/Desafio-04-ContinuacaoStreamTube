...
- **Video Worker** (FFmpeg) → consumes BullMQ jobs from Redis queue, processes videos (ffprobe metadata + ffmpeg thumbnails), updates DB and storage
- **Database** (PostgreSQL) → users, channels, videos, comments, likes
- **Object Storage** (MinIO / S3-compatible) → video files and thumbnails; organized as `channels/{channelId}/videos/{videoId}/original` and `.../thumbnail.jpg`
- **Message Queue** (BullMQ + Redis) → `video-processing` queue; job payload: `{ videoId, storageKey }`
- **Email Service** (SMTP / Mailpit in dev) → account confirmation and password recovery

## Videos Module (Phase 03)

`nestjs-project/src/videos/` — upload, processing, streaming, and download.
...
**Video lifecycle:** `draft` → (complete called) → `processing` → (worker done) → `ready` / `error`
...
- `video-worker` — FFmpeg worker container (Dockerfile.worker.dev)
